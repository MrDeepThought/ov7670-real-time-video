...
OV7670::OV7670(Mode m, const int SIOD, const int SIOC, const int VSYNC, const int HREF, const int XCLK, const int PCLK, const int D0, const int D1, const int D2, const int D3, const int D4, const int D5, const int D6, const int D7)
  :i2c(SIOD, SIOC)
{
  bool clkOk = ClockEnable(XCLK, 20000000); //base is 80MHz
  Serial.printf("XCLK LEDC init: %s\n", clkOk ? "ok" : "FAILED");

  // SCCB liveness probe: any ACKed write means the camera answers on the bus.
  bool sccbOk = i2c.writeRegister(ADDR, REG_COM7, 0x80); // soft reset
  Serial.printf("SCCB probe (soft reset): %s\n", sccbOk ? "ACK" : "NO ACK - check SIOD/SIOC wiring + 4.7k pull-ups");
  delay(2); // datasheet: >1ms settling after soft reset

  Serial.print("Waiting for VSYNC...");
  pinMode(VSYNC, INPUT);
  uint32_t t0 = millis();
  while(!digitalRead(VSYNC)) if(millis() - t0 > 2000) { Serial.println(" TIMEOUT (VSYNC stuck low - no XCLK at camera? VSYNC wire off? camera unpowered?)"); break; }
  t0 = millis();
  while(digitalRead(VSYNC))  if(millis() - t0 > 2000) { Serial.println(" TIMEOUT (VSYNC stuck high)"); break; }
  Serial.println(" done");
