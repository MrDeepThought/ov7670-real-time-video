...
void I2SCamera::i2sRun()
{
    ...
    // Align DMA start to the VSYNC falling edge. Tight spin (sub-frame, no yield
    // so we don't miss the edge), but time-bounded so a lost VSYNC can't wedge
    // the core.
    uint32_t t0 = millis();
    while (gpio_get_level(vSyncPin) == 0) if (millis() - t0 > 1000) break;
    t0 = millis();
    while (gpio_get_level(vSyncPin) != 0) if (millis() - t0 > 1000) break;
...
{    
  ...
  gpio_config_t conf = {
    ...
    // gpio_matrix_in() was removed from the public API in ESP-IDF 5.x; the ROM
    // helper esp_rom_gpio_connect_in_signal() takes the same (gpio, signal, invert).
    esp_rom_gpio_connect_in_signal(D0,    I2S0I_DATA_IN0_IDX, false);
    esp_rom_gpio_connect_in_signal(D1,    I2S0I_DATA_IN1_IDX, false);
    esp_rom_gpio_connect_in_signal(D2,    I2S0I_DATA_IN2_IDX, false);
    esp_rom_gpio_connect_in_signal(D3,    I2S0I_DATA_IN3_IDX, false);
    esp_rom_gpio_connect_in_signal(D4,    I2S0I_DATA_IN4_IDX, false);
    esp_rom_gpio_connect_in_signal(D5,    I2S0I_DATA_IN5_IDX, false);
    esp_rom_gpio_connect_in_signal(D6,    I2S0I_DATA_IN6_IDX, false);
    esp_rom_gpio_connect_in_signal(D7,    I2S0I_DATA_IN7_IDX, false);
    esp_rom_gpio_connect_in_signal(0x30,  I2S0I_DATA_IN8_IDX, false);
    esp_rom_gpio_connect_in_signal(0x30,  I2S0I_DATA_IN9_IDX, false);
    esp_rom_gpio_connect_in_signal(0x30,  I2S0I_DATA_IN10_IDX, false);
    esp_rom_gpio_connect_in_signal(0x30,  I2S0I_DATA_IN11_IDX, false);
    esp_rom_gpio_connect_in_signal(0x30,  I2S0I_DATA_IN12_IDX, false);
    esp_rom_gpio_connect_in_signal(0x30,  I2S0I_DATA_IN13_IDX, false);
    esp_rom_gpio_connect_in_signal(0x30,  I2S0I_DATA_IN14_IDX, false);
    esp_rom_gpio_connect_in_signal(0x30,  I2S0I_DATA_IN15_IDX, false);

    esp_rom_gpio_connect_in_signal(VSYNC, I2S0I_V_SYNC_IDX, true);
    esp_rom_gpio_connect_in_signal(0x38,  I2S0I_H_SYNC_IDX, false);  //0x30 sends 0, 0x38 sends 1
    esp_rom_gpio_connect_in_signal(HREF,  I2S0I_H_ENABLE_IDX, false);
    esp_rom_gpio_connect_in_signal(PCLK,  I2S0I_WS_IN_IDX, false);
